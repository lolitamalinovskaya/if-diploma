...
export default function Section3Modniky_All_Items({state, updateState}) {
  ...
  const items = [...data].splice(0, 4).map((el) => {
    ...
    return <Link to={`/product/${el.id}`} style={{textDecoration: 'none', color: 'unset', margin: 'unset', width: '25%'}}><Section3Modniky_1_Item price={price} priceBeforeSale={priceBeforeSale} sale={17} imageURL={image}/></Link>
  })
